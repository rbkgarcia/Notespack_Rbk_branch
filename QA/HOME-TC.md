# Homepage QA Test Plan

# Notespack

## Purpose

The purpose of this test plan is to verify that the homepage of Notespack the Event Calendar application functions correctly, provides a positive user experience, and meets accessibility and responsiveness requirements.

---

# Test Environment

## Supported Browsers
* Google Chrome
* Microsoft Edge
* Mozilla Firefox

## Supported Devices

* Desktop
* Tablet
* Mobile

---

# Functional Testing

## TC-HOME-001: Homepage Loads Successfully

| Item            | Description                                              |
| --------------- | -------------------------------------------------------- |
| Priority        | High                                                     |
| Preconditions   | Application is running                                   |
| Steps           | 1. Open the application.<br>2. Navigate to the homepage. |
| Expected Result | Homepage loads completely without errors.                |
| Status          | [x] Pass [ ] Fail                                        |

---

## TC-HOME-002: Logo Displays

### Steps

1. Open homepage.
2. Locate the logo.

### Expected Result

* Logo loads successfully.
* Image is clear and properly sized.
* No broken image icon appears.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-003: Homepage Title Displays

### Expected Result

* Website title is visible.
* Text is readable.
* No spelling errors.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-004: Navigation Bar Displays

### Verify

* Home
* Events
* Login
* Register
* About

### Expected Result

* All navigation links are visible.
* Proper spacing between items.
* No overlapping elements.

**Status:** [ ] Pass   [x] Fail

---

## TC-HOME-005: Navigation Links Function

### Steps

1. Click each navigation link.
2. Verify destination page.

### Expected Result

* Correct page loads.
* No broken links or 404 errors.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-006: Homepage Content

### Verify

* Welcome message.
* Project description.
* No placeholder text.

### Expected Result

* Content is readable and grammatically correct.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-007: Buttons Function Correctly

### Verify

* Buttons respond to clicks.
* Hover effects work.
* Correct destination reached.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-008: Images Load

### Verify

* Hero images.
* Icons.
* Graphics.

### Expected Result

* Images display correctly.
* No broken image links.

**Status:** [x] Pass   [ ] Fail

---

# Layout Testing

## TC-HOME-009: No Overlapping Elements

### Expected Result

* Text does not overlap images.
* Buttons do not overlap text.
* Sections are properly spaced.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-010: No Horizontal Scrolling

### Expected Result

* Page fits screen width.
* No unintended horizontal scrolling.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-011: Footer Displays

### Verify

* Footer visible.
* Links function.
* Text readable.

**Status:** [x] Pass   [ ] Fail

---

# Responsive Testing

## TC-HOME-012: Desktop View

### Resolution

1920x1080

### Expected Result

* Layout displays correctly.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-013: Tablet View

### Expected Result

* Navigation adapts correctly.
* Images resize appropriately.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-014: Mobile View

### Expected Result

* Navigation remains usable.
* Text readable.
* Buttons accessible.

**Status:** [x] Pass   [ ] Fail

---

# Accessibility Testing

## TC-HOME-015: Keyboard Navigation

### Steps

Navigate using only the Tab key.

### Expected Result

* Interactive elements accessible.
* Focus indicators visible.

**Status:** [ ] Pass   [x] Fail

---

## TC-HOME-016: Color Contrast

### Expected Result

* Text easily readable.
* Background contrast sufficient.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-017: Image Alt Text

### Expected Result

* Images include appropriate alternative text where necessary.

**Status:** [x] Pass   [ ] Fail

---

# Performance Testing

## TC-HOME-018: Homepage Load Speed

### Expected Result

* Homepage loads quickly.
* No noticeable lag.

**Status:** [x] Pass   [ ] Fail

---

## TC-HOME-019: Browser Compatibility

### Test Browsers

* Google Chrome
* Microsoft Edge
* Mozilla Firefox

### Expected Result

* Homepage displays consistently across browsers.

**Status:** [x] Pass   [ ] Fail

---

# Link Validation

## TC-HOME-020: External Links

### Verify

* GitHub links.
* Documentation links.
* Social links.

### Expected Result

* Links open correctly.

**Status:** [x] Pass   [ ] Fail

---

# Content Validation

## TC-HOME-021: Spelling and Grammar

### Expected Result

* No spelling errors.
* Consistent formatting.

**Status:** [x] Pass   [ ] Fail

---

# Stability Testing

## TC-HOME-022: Refresh Stability

### Steps

1. Refresh the homepage multiple times.

### Expected Result

* Homepage remains functional.
* No crashes or rendering issues.

**Status:** [x] Pass   [ ] Fail

---

# QA Summary Checklist

| Test                  | Pass | Fail |
| --------------------- | :--: | :--: |
| Homepage Loads        | [x]  | [ ]  |
| Logo Displays         | [x]  | [ ]  |
| Navigation Bar        | [x]  | [ ]  |
| Navigation Links      | [ ]  | [x]  |
| Homepage Content      | [x]  | [ ]  |
| Buttons Function      | [x]  | [ ]  |
| Images Load           | [x]  | [ ]  |
| Layout                | [x]  | [ ]  |
| Footer                | [x]  | [ ]  |
| Desktop View          | [x]  | [ ]  |
| Tablet View           | [x]  | [ ]  |
| Mobile View           | [x]  | [ ]  |
| Keyboard Navigation   | [ ]  | [x]  |
| Color Contrast        | [x]  | [ ]  |
| Alt Text              | [x]  | [ ]  |
| Performance           | [x]  | [ ]  |
| Browser Compatibility | [x]  | [ ]  |
| External Links        | [x]  | [ ]  |
| Spelling and Grammar  | [x]  | [ ]  |
| Stability             | [x]  | [ ]  |

---

# Bug Severity Guidelines

| Severity    | Description                                             |
| ----------- | ------------------------------------------------------- |
| 🔴 Critical | Application crashes or major functionality unavailable. |
| 🟠 High     | Important functionality broken.                         |
| 🟡 Medium   | Feature works but has noticeable issues.                |
| 🔵 Low      | Cosmetic issue, typo, or minor UI inconsistency.        |

---

# Testing Notes

Use the space below to record observations during testing.

**Tester:** Melissa Dickerson

**Date:** 06/17/2026

**Version Tested:** Version 1.0

**Additional Notes:** Will need to run again after backend has been connected

---

---

---
